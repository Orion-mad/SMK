...
try {
    ...
    $stmt = $db->prepare("
        SELECT 
            p.*,c.razon_social AS razon_social,
            CASE 
...
*/    
    ...
    if (!empty($presupuesto['condiciones'])) {
        ...
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->SetTextColor(31, 73, 125);
        $pdf->Cell(0, 8, utf8_decode('Condiciones Especiales'), 0, 1, 'L');
    ...
    $terminos = [
        ...
        'La forma de pago 50% al iniciar el trabajo, saldo al finalizar.',
    ...
    // Salida del PDF
    $filename = __DIR__.'/Presupuesto_' . $presupuesto['razon_social'] . '.pdf';
