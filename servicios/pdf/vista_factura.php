...
require('../phpqrcode/qrlib.php');
    if($_SESSION['facturacion']['PTOVTA_PROD'] == 'producción'):
        $_TBL               = "facturacion_arca";                       // TABLA DE LISTADO                      
    else:
        $_TBL               = "facturacion";                       // TABLA DE LISTADO                      
    endif;
...
/*   
echo'<pre>';print_r($cadathumb);echo'</pre>1<hr>';
...
echo'<pre>';print_r($_FC);echo'</pre>2<hr>';
echo'<pre>';print_r($FC_det->Iva->AlicIva->Id);echo'</pre>3<hr>';
echo'<pre>';print_r($_FC['concepto']);echo'</pre>4<hr>';
die;
...
//echo'<pre>';print_r($_SESSION['ga']);echo'</pre>';
...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(50, 6, 'FACTURA '.$this->tf, 0, 2, 'R');
        if($this->tf2):
            $this->SetFont('Arial', '', 10);
...
$tf         = $_FC['tipo_factura'];
$valor = empty($FC_det->ImpOpEx) 
  ? (float)($FC_det->ImpNeto ?? 0) 
  : (float)$FC_det->ImpOpEx;
...
if($FC_det->CondicionIVAReceptorId == 5):
    $TR = 'Consumidor Final';
elseif($FC_det->CondicionIVAReceptorId == 4):
...
$pdf->Cell(120, 8, utf8_decode('Periodo facturado desde '.$GUSERS->fechaArca($FC_det->FchServDesde).' hasta: '.$GUSERS->fechaArca($FC_det->FchServHasta)), 1, 0);
//$pdf->Cell(65, 8, utf8_decode('Hasta: '.$GUSERS->fechaArca($FC_det->FchServHasta)), 'B T', 0);
$pdf->Cell(70, 8, utf8_decode('Fecha venc. para el pago: '.$GUSERS->fechaArca($FC_det->FchVtoPago)), 1, 1);
$pdf->Ln(3);
// Cliente
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(190, 6, utf8_decode('Razón Social: '), 'T R L',1);
$pdf->SetFont('Arial', '', 10);
$pdf->MultiCell(190, 6, utf8_decode($_FC['razon_social']), 'L R', 1);

$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(20, 6, utf8_decode('Dirección: '), 'L', 0);
$pdf->SetFont('Arial', '', 10);
$pdf->Cell(80, 6, utf8_decode($FC_det->Domicilio), '', 0);
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(30, 6, 'CUIT/CUIL: ', '', 0,'R');
$pdf->SetFont('Arial', '', 10);
$pdf->Cell(60, 6, $FC_det->DocNro, 'R', 1,'L');

$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(27, 6,  utf8_decode('Condición IVA: '), 'L B', 0);
$pdf->SetFont('Arial', '', 10);
$pdf->Cell(163, 6,  utf8_decode($TR), 'B R', 1,'L');
$pdf->Ln(3);
