...
require('../phpqrcode/qrlib.php');
    $_FC = $USERS->full_list("facturacion","WHERE id  = {$_GET['factura']}");
...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(50, 5, 'Condicion de Venta: Contado | Comprobante Nro: '.$this->ptv.'-'.$this->cmp, 0, 2, 'R');
    ...
    function DetalleFactura($items)
    {
        ...
        foreach ($items as $item) {
            ...
            $this->MultiCell($wDesc, 6, $item['descripcion'], 'B');

            // Calcular altura usada por la MultiCell
            $h = $this->GetY() - $y;

            // Volver a la posición de la primera fila
            $this->SetXY($x + $wDesc, $y);
            $this->Cell($wCant, $h, $item['cantidad'], 0, 0, 'R');
            $this->Cell($wPrecio, $h, number_format($item['precio'],2), 0, 0, 'R');
            $this->Cell($wTotal, $h, number_format($item['total'],2), 0, 1, 'R');

            // Chequear si queda espacio, si no: nueva página
            if ($this->GetY() > 200) {
                $this->AddPage();
                $this->Ln(10);
            }
...
*/
$detalle = [];
$text_detalle =  utf8_decode(strip_tags($FC_det->Detalle));
for ($i = 1; $i <= 20; $i++) {
    $detalle[] = [
        'descripcion' =>$text_detalle,
        'cantidad' => 1,
        'precio' => $parcial,
        'total' => $total
    ];
